# SealBid Architecture Design Document

## 📋 Overview

SealBid is a decentralized privacy auction platform based on Sui blockchain and Seal time-lock encryption. This document details the system architecture design, data flow, and key technical decisions.

## 🏛️ System Architecture

```
┌─────────────────────────────────────────────────────────────┐
│                         User Interface                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│  │Create    │  │Create    │  │Submit    │  │View      │   │
│  │Token     │  │Auction   │  │Bid       │  │Results   │   │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘   │
└───────┼─────────────┼─────────────┼─────────────┼──────────┘
        │             │             │             │
        ▼             ▼             ▼             ▼
┌─────────────────────────────────────────────────────────────┐
│                    Frontend Application (React)             │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  Sui SDK  │  Seal SDK  │  Wallet Kit  │  State Mgmt │  │
│  └──────────────────────────────────────────────────────┘  │
└────────────────────────┬────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────┐
│                      Sui Blockchain                         │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │ coin_factory │  │   auction    │  │seal_integration│    │
│  └──────────────┘  └──────────────┘  └──────────────┘     │
└────────────────────────┬────────────────────────────────────┘
                         │
                         ▼
┌─────────────────────────────────────────────────────────────┐
│                     Seal Service Layer                      │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │  Key Server 1│  │  Key Server 2│  │  Key Server N│     │
│  └──────────────┘  └──────────────┘  └──────────────┘     │
└─────────────────────────────────────────────────────────────┘
```

## 🔄 核心流程

### 1. 代币创建流程

```
用户输入代币信息
    ↓
部署包含 OTW 的模块
    ↓
调用 coin_factory::create_coin
    ↓
创建 TreasuryCap 和 CoinMetadata
    ↓
将代币注册到 CoinRegistry
    ↓
返回 TreasuryCap 给创建者
```

### 2. 拍卖创建流程

```
用户设置拍卖参数
    ↓
验证参数有效性
    ↓
调用 auction::create_auction
    ↓
创建 Auction 对象
    ↓
存储 TreasuryCap
    ↓
发出 AuctionCreated 事件
    ↓
共享 Auction 对象
```

### 3. 出价流程（核心）

```
用户填写出价金额
    ↓
前端使用 Seal 加密
    │
    ├─ 生成 keyId = BCS(end_time)
    │
    ├─ 调用 Seal SDK 加密出价
    │
    └─ 得到加密数据
    ↓
提交交易到链上
    │
    ├─ 传入加密数据
    │
    ├─ 附带 SUI 保证金
    │
    └─ 存储到 Auction 对象
    ↓
发出 BidPlaced 事件
```

### 4. 拍卖完成和分配流程

```
等待结束时间到达
    ↓
创建者调用 finalize_auction
    ↓
验证时间和权限
    ↓
标记拍卖为已完成
    ↓
发出 AuctionFinalized 事件
    ↓
解密所有出价
    │
    ├─ 调用 Seal 服务
    │
    ├─ 验证访问策略 (seal_approve)
    │
    └─ 获取解密后的出价
    ↓
根据策略排序选择中标者
    │
    ├─ TOP_N: 按金额降序
    │
    ├─ RANDOM_N: 随机选择
    │
    └─ CLOSEST_TO_AVG: 计算平均值
    ↓
分配代币给中标者
    ↓
退款给未中标者
```

## 🔐 Seal 时间锁加密设计

### 密钥 ID 格式

```
keyId = BCS.encode(end_time: u64)
```

完整的密钥标识符由 Seal 服务器自动添加包 ID 前缀：

```
full_key_id = [package_id] + keyId
```

### 访问策略实现

```move
entry fun seal_approve(id: vector<u8>, c: &Clock) {
    // 解析 keyId
    let mut bcs_data = bcs::new(id);
    let end_time = bcs::peel_u64(&mut bcs_data);
    let leftovers = bcs::into_remainder_bytes(bcs_data);

    // 验证格式
    assert!(leftovers.length() == 0, ENoAccess);
    
    // 验证时间
    assert!(clock::timestamp_ms(c) >= end_time, ENoAccess);
}
```

### 加密解密流程

```
加密（用户出价时）：
plaintext (出价) → Seal.encrypt(data, keyId) → ciphertext

解密（拍卖结束后）：
ciphertext → Seal.decrypt(data, keyId, proof) → plaintext

其中 proof 是证明当前时间 >= end_time 的凭证
```

## 📊 数据模型

### Auction 对象结构

```move
public struct Auction<phantom T> has key {
    id: UID,
    creator: address,              // 创建者地址
    coin_name: String,             // 代币名称
    total_supply: u64,             // 总供应量
    winner_count: u64,             // 中标者数量
    strategy: u8,                  // 分配策略
    start_time: u64,               // 开始时间（毫秒）
    end_time: u64,                 // 结束时间（毫秒）
    encrypted_bids: vector<EncryptedBid>,  // 加密出价列表
    payment_pool: Balance<SUI>,    // 支付池
    finalized: bool,               // 是否已完成
    treasury_cap: TreasuryCap<T>,  // 代币铸造权
}
```

### EncryptedBid 结构

```move
public struct EncryptedBid has store, drop, copy {
    bidder: address,           // 出价者地址
    encrypted_data: vector<u8>, // 加密的出价数据
    payment_amount: u64,       // 支付的保证金
}
```

### BidInfo 结构（解密后）

```move
public struct BidInfo has store, drop, copy {
    bidder: address,        // 出价者地址
    bid_amount: u64,        // 出价金额
    payment_amount: u64,    // 支付金额
}
```

## 🎯 分配策略算法

### 1. TOP_N 策略

```
1. 解密所有出价
2. 按 bid_amount 降序排序
3. 选择前 winner_count 个
4. 平均分配 total_supply
```

伪代码：
```python
def top_n_strategy(bids, winner_count, total_supply):
    sorted_bids = sort(bids, key=lambda x: x.bid_amount, reverse=True)
    winners = sorted_bids[:winner_count]
    token_per_winner = total_supply / winner_count
    
    for winner in winners:
        distribute(winner.address, token_per_winner)
    
    for loser in sorted_bids[winner_count:]:
        refund(loser.address, loser.payment_amount)
```

### 2. RANDOM_N 策略

```
1. 解密所有出价
2. 使用链上随机数或哈希
3. 随机选择 winner_count 个
4. 平均分配 total_supply
```

伪代码：
```python
def random_n_strategy(bids, winner_count, total_supply, seed):
    shuffled = shuffle(bids, seed)
    winners = shuffled[:winner_count]
    token_per_winner = total_supply / winner_count
    
    for winner in winners:
        distribute(winner.address, token_per_winner)
    
    for loser in shuffled[winner_count:]:
        refund(loser.address, loser.payment_amount)
```

### 3. CLOSEST_TO_AVG 策略

```
1. 解密所有出价
2. 计算平均值 avg = sum(bids) / count(bids)
3. 按 |bid_amount - avg| 升序排序
4. 选择前 winner_count 个（最接近平均值）
5. 平均分配 total_supply
```

伪代码：
```python
def closest_to_avg_strategy(bids, winner_count, total_supply):
    avg = sum(bid.bid_amount for bid in bids) / len(bids)
    
    sorted_bids = sort(bids, key=lambda x: abs(x.bid_amount - avg))
    winners = sorted_bids[:winner_count]
    token_per_winner = total_supply / winner_count
    
    for winner in winners:
        distribute(winner.address, token_per_winner)
    
    for loser in sorted_bids[winner_count:]:
        refund(loser.address, loser.payment_amount)
```

## 🔒 安全设计

### 1. 时间验证

- 创建拍卖时：`start_time < end_time`
- 提交出价时：`current_time >= start_time && current_time < end_time`
- 完成拍卖时：`current_time >= end_time`

### 2. 权限控制

- 创建拍卖：任何人
- 提交出价：任何人（拍卖期间）
- 完成拍卖：仅创建者
- 分配代币：仅创建者（拍卖完成后）

### 3. 状态管理

- 使用 `finalized` 标志防止重复完成
- 使用时间戳确保操作时序正确
- 使用 TreasuryCap 控制代币铸造

### 4. 金额安全

- 验证支付金额 > 0
- 使用 Balance 类型管理资金池
- 确保退款金额不超过池子余额

## 📈 性能优化

### 1. 链上优化

- 使用 vector 存储出价（动态数组）
- 使用事件而非存储记录历史
- 共享对象模式减少拥有权转移

### 2. 前端优化

- SealClient 实例复用
- SessionKey 缓存减少签名次数
- 批量查询减少 RPC 调用

### 3. 查询优化

- 使用事件索引快速检索
- 缓存常用查询结果
- 分页加载大列表

## 🔄 未来扩展

### 1. 链上解密

利用 Seal 的链上解密功能：

```move
use seal::bf_hmac_encryption;

// 验证派生密钥
let verified_keys = bf_hmac_encryption::verify_derived_keys(
    derived_keys,
    package_id,
    identity,
    public_keys
);

// 执行解密
let decrypted = bf_hmac_encryption::decrypt(
    encrypted_object,
    verified_keys,
    public_keys
);
```

### 2. 动态策略

允许用户自定义分配策略：

```move
public struct CustomStrategy {
    weight_function: vector<u8>,  // 自定义权重函数
    selection_logic: u8,          // 选择逻辑
}
```

### 3. 多轮拍卖

支持分批次、多轮次的拍卖：

```move
public struct MultiRoundAuction {
    rounds: vector<Round>,
    current_round: u64,
}
```

### 4. NFT 拍卖

扩展支持 NFT 的拍卖：

```move
public struct NFTAuction<phantom T: key + store> {
    nfts: vector<T>,
    // ...
}
```

## 📚 参考资料

- [Sui Move Book](https://move-book.com)
- [Seal Documentation](https://seal-docs.wal.app/)
- [Sui Object Model](https://docs.sui.io/concepts/object-model)
- [Time Lock Encryption Pattern](https://github.com/MystenLabs/seal/blob/main/move/patterns/sources/tle.move)

---

**本文档持续更新中**

